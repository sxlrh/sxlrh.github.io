...
window.globalSearch = function() {
    ...
    if (keyword.startsWith('#')) {
        // 搜索话题
        searchTopic(keyword.substring(1));
    } else if (keyword.startsWith('@')) {
        // 搜索用户
        searchUser(keyword.substring(1));
    } else {
        // 搜索帖子内容
        searchPosts(keyword);
    }
...
async function searchPosts(keyword) {
    ...
    try {
        ...
        if (!data || data.length === 0) {
            showToast(`没有找到包含 "${keyword}" 的帖子`, 'info');
        } else {
            showSearchResults(data, keyword);
        }
    ...
    try {
        ...
        if (!data || data.length === 0) {
            const { data: fuzzyData } = await supabase
                .from('posts')
                .select('*')
                .contains('content', [`#${topic}`])
                .order('created_at', { ascending: false })
                .limit(20);
            
            if (fuzzyData && fuzzyData.length > 0) {
                showSearchResults(fuzzyData, `#${topic}`);
            } else {
                showToast(`没有找到包含 #${topic} 的帖子`, 'info');
            }
        } else {
            showSearchResults(data, `#${topic}`);
        }
...
window.searchUser = async function(username) {
    ...
    try {
        ...
        if (!data || data.length === 0) {
            showToast(`没有找到用户: @${username}`, 'info');
            return;
        }
        
        // 如果只找到一个用户，直接打开主页
        if (data.length === 1) {
            openUserProfile(data[0].id);
        } else {
            // 多个用户显示选择
            showUserSearchResults(data, username);
        }
...
// 显示搜索结果
function showSearchResults(posts, keyword) {
    // 创建搜索结果模态框
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 600px;">
                <div class="modal-header">
                    <h3 id="search-results-title">搜索结果</h3>
                    <span class="close-btn" onclick="closeSearchResults()">&times;</span>
                </div>
                <div class="modal-body" id="search-results-container" style="max-height: 500px; overflow-y: auto;">
                </div>
            </div>
        `;
    ...
    document.getElementById('search-results-title').textContent = `搜索: ${keyword}`;
    const container = document.getElementById('search-results-container');
    
    if (!posts || posts.length === 0) {
        container.innerHTML = '<p style="text-align:center;color:#999;padding:20px;">没有找到相关帖子</p>';
    } else {
        container.innerHTML = '';
        posts.forEach(post => {
            const div = document.createElement('div');
            div.style.cssText = 'padding:15px;border-bottom:1px solid #e0e0e0;cursor:pointer;';
            div.innerHTML = `
                <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                    <img src="${post.user_avatar}" style="width:24px;height:24px;border-radius:50%;">
                    <span style="font-weight:600;color:#667eea;font-size:0.9rem;">${post.username}</span>
                </div>
                <div style="color:#333;margin-bottom:8px;">${escapeHtml(post.content || '').substring(0, 100)}${(post.content?.length || 0) > 100 ? '...' : ''}</div>
...
// 显示用户搜索结果
function showUserSearchResults(users, keyword) {
    let modal = document.getElementById('user-search-modal');
    if (!modal) {
        modal = document.createElement('div');
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 400px;">
                <div class="modal-header">
                    <h3>选择用户</h3>
                    <span class="close-btn" onclick="closeUserSearch()">&times;</span>
                </div>
                <div class="modal-body" id="user-search-results" style="max-height: 400px; overflow-y: auto;">
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    users.forEach(user => {
        const div = document.createElement('div');
        div.style.cssText = 'padding:15px;border-bottom:1px solid #e0e0e0;cursor:pointer;display:flex;align-items:center;gap:10px;';
        div.innerHTML = `
